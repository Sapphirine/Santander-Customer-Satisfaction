...
#Load Models
tmp1 <- fread('./output/models_train.csv')
tmp2 <- fread('./output/models_test.csv')
models <- rbind( tmp1, tmp2  )

#Include ID
raw <- data.table( ID=c(train$ID, test$ID)  )
raw <- merge(raw, models, by="ID", sort=F )
...
summary( sub$TARGET  )
write.table( sub, './submission/final_result.csv', row.names=F, quote=F, sep=','  )
